...
export const getPropertyValue = <Object, Key extends keyof Object>(
  object: Object,
  propertyName: Key
): Object[Key] => object[propertyName];
...
export const randomDecimalString = (
  min: number,
  max: number,
  fixed: number = 2
): string => randomFloat(min, max).toFixed(fixed);

export const randomDecimal = (
  min: number,
  max: number,
  fixed: number = 2
): number => +randomDecimalString(min, max, fixed);

export const randomInt = (min: number, max: number): number =>
  Math.floor(randomFloat(min, max));

export const randomIntAsString = (
  min: number,
  max: number,
  padStart: number = 2
): string => {
...
export const randomValue = <T extends any>(
  options: T | T[],
  inNumber?: number
): T | undefined => {
...
export const calculateAvailablePages = <T extends any>(
  data: T[],
  pageSize: number = 25
...
export const randomSetOfData = <T extends any>(
  data: T[],
